...
def gen_sensor_calibration(calibrations, calibration_file_path):
    ...
    for channel, calibrated_sensor in calibrations.items():
        if channel.startswith('CAM'):
            gen_camera_params(channel, calibrated_sensor,
            ...
            if channel == 'LIDAR_TOP':
                channel = 'velodyne32'
            #############################
            gen_velodyne_params(channel, calibrated_sensor,
                                calibration_file_path)
...
def convert_calibration(dataset_path, calibration_root_path):
    ...
    Args:
        ...
        calibration_root_path (str): sensor calibrations saved path
    """
    DATAVERSION = 'v1.0-trainval'  # Change if you want to use the v1.0-mini
    nuscenes_schema = NuScenesSchema(
        dataroot=dataset_path, version=DATAVERSION)
